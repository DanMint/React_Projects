...
function SelectFood() {
    ...
    // State to keep track of breakfast choices
    const [breakfastChoice, setBreakfastChoice] = useState([]);

    function handleBreakfastChange(foodArray, event) {
        const selectedFood = foodArray[event.target.value];
        setBreakfastChoice([...breakfastChoice, selectedFood]);
    ...
    return (
        <>
            ...
            <div className={styles.food}>
                {/* Display sections for meats, carbohydrates, and fiber */}
                <h2>Meats:</h2>
                <ul>
                    {food_meat.map(food => (
            ...
            {/* Additional sections for lunch and dinner */}
        </>
    );
}
